    ...
    try:
        ...
        for d in data:
            ...
            row = {
                ...
                "SQUARE_FOOTAGE": int(d.get("squareFootage", 0) or 0),
        ...
        # Log audit entry
        audit_query = """
                INSERT INTO audit_logs_rental_inactiive (ingestion_id, source, ingestion_timestamp, record_count, status,city)
        ...
        # Log failure
        audit_query = """
            INSERT INTO audit_logs_rental_inactiive (ingestion_id, source, ingestion_timestamp, record_count, status, error_message,city)
...
def rent_inactive_main():
    ...
    audit_logs_rental_inactiive = """
                SELECT city, SUM(record_count) AS total_records
                FROM audit_logs_rental_inactiive
                WHERE city = %s
                GROUP BY city
    ...
    check_records = """
                        ...
                        FROM audit_logs_rental_inactiive) t 
    ...
    results = []
    for city in cities:
        cursor.execute(audit_logs_rental_inactiive, (city,))  # Use a list, not a tuple
